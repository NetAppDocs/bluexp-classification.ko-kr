...
[role="lead"]
BlueXP 작업 공간에서 Cloud Data Sense가 스캔하는 첫 1TB의 데이터는 무료입니다. 해당 시점 이후에도 데이터를 계속 스캔하려면 NetApp의 BYOL 라이센스 또는 클라우드 공급자 마켓플레이스의 가입형 라이센스가 필요합니다.
...
.단계
. BlueXP 왼쪽 탐색 메뉴에서 * Governance > Digital Wallet * 을 클릭한 다음 * Data Services Licenses * 탭을 선택합니다.
. 라이선스 추가 * 를 클릭합니다.
...
+
.. 가능합니다  your Cloud Data Sense license file,NetApp Support 사이트에서 라이센스 파일을 받으십시오.
...
BYOL 라이센스를 갱신하면 BlueXP는 Digital Wallet의 라이센스를 자동으로 업데이트하고 모든 대시보드에 대한 모든 액세스를 제공합니다. BlueXP가 보안 인터넷 연결(예: 어두운 사이트에 설치된 경우)을 통해 라이센스 파일에 액세스할 수 없는 경우 직접 파일을 얻고 BlueXP에 수동으로 업로드할 수 있습니다. 자세한 내용은 을 참조하십시오  a Cloud Data Sense BYOL license,Cloud Data Sense 라이센스를 업데이트하는 방법.
