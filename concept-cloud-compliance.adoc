...
* 파일 공유(예: Dell EMC Isilon, Pure Storage, Nutanix 등)
* 데이터베이스:
+
...
BlueXP 분류는 이제 최대 500TiB의 데이터에 무료로 사용할 수 있습니다. 분류 라이선스 또는 유료 가입이 필요하지 않습니다. 스캔하려는 데이터가 500TiB를 넘는 경우 원본 시스템 중 일부에서 스캔을 비활성화하고 500TiB 제한을 초과할 수 있는 새로운 시스템을 스캔할 수 있습니다.
