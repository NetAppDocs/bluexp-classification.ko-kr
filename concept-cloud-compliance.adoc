...
* 파일에 특정 PII가 포함된 경우 이메일을 통해 특정 사용자에게 알립니다(을 사용하여 이 기준을 정의합니다 link:task-org-private-data.html#controlling-your-data-using-policies["정책"^]) 따라서 실천 계획을 결정할 수 있습니다.
...
* 클라우드 데이터 센스를 사용하는 비용은 스캔하는 데이터의 양에 따라 다릅니다. BlueXP 작업 공간에서 Data Sense가 스캔하는 첫 1TB의 데이터는 30일간 무료로 제공됩니다. 여기에는 모든 작업 환경 및 데이터 소스의 모든 데이터가 포함됩니다. AWS, Azure 또는 GCP Marketplace에 대한 가입 또는 NetApp의 BYOL 라이센스를 구입해야 하며, 이후 계속해서 데이터를 스캔할 수 있습니다. 을 참조하십시오 https://cloud.netapp.com/netapp-cloud-data-sense["가격"^] 를 참조하십시오.
+
...
데이터 센스(Data Sense)는 스캐너 노드당 하루에 15-40TB의 데이터를 스캔할 수 있습니다. 스캔 속도는 네트워크 지연 시간, 디스크 지연 시간, 네트워크 대역폭, 환경 크기 및 파일 배포 크기의 영향을 받습니다.

link:task-deploy-compliance-onprem.html#deploy-data-sense-on-premises["데이터를 스캔하기 위해 여러 스캐너 노드를 구축하는 방법에 대해 자세히 알아보십시오"^].
...
이름 요소 인식:: Cloud Data Sense는 AI를 사용하여 문서에서 자연인의 이름을 추출합니다. link:task-responding-to-dsar.html["데이터 주체 액세스 요청에 응답하는 방법에 대해 알아봅니다"^].
