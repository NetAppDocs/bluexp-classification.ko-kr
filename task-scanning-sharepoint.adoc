...
summary: Cloud Data Sense를 사용하여 SharePoint Online 계정의 파일 스캔을 시작하려면 몇 단계를 완료하십시오. 
---
= SharePoint 계정 스캔 중
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
Cloud Data Sense를 사용하여 SharePoint Online 계정의 파일 스캔을 시작하려면 몇 단계를 완료하십시오.
...
인터넷에 연결되지 않은 사이트에 데이터 센스를 설치하는 경우, 인터넷에 연결되지 않은 동일한 사이트에 Cloud Manager Connector도 설치해야 합니다. https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-install-connector-onprem-no-internet.html["자세한 정보"^].
