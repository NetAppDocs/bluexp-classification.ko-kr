...
을 참조하십시오 link:concept-cloud-compliance.html["지원되는 작업 환경 및 데이터 소스"].
...
BlueXP 분류는 개인 정보(PII) 및 민감한 개인 정보(SPii)와 같은 다양한 유형의 프라이빗 데이터를 식별할 수 있습니다. 개인 데이터에 대한 자세한 내용은 을 참조하십시오 https://docs.netapp.com/us-en/bluexp-classification/reference-private-data-categories.html["BlueXP 분류가 검사하는 프라이빗 데이터의 범주입니다"].
