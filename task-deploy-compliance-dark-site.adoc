...
summary: 인터넷 액세스가 없는 사내 사이트의 Linux 호스트에 Data Sense를 설치하려면 몇 가지 단계를 완료하십시오. 이러한 유형의 설치는 보안 사이트에 적합합니다. 
---
= 인터넷에 액세스할 수 없는 호스트에 데이터 센스를 설치합니다
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
인터넷 액세스가 없는 사내 사이트의 Linux 호스트에 Data Sense를 설치하려면 몇 가지 단계를 완료하십시오. 이러한 유형의 설치는 보안 사이트에 적합합니다.

참고: 또한 이 기능을 사용할 수 있습니다 link:task-quick-start-connector-on-prem.html["인터넷에 액세스할 수 있는 온프레미스 사이트에 데이터 센스를 구현합니다"].
...
현재 Cloud Volumes ONTAP, Azure NetApp Files, FSx for ONTAP, AWS S3 또는 Google Drive 스캔을 지원하지 않습니다. 데이터 센스를 어두운 사이트에 배포할 때 OneDrive 또는 SharePoint Online 계정
...
* 스캔하려는 데이터 세트의 크기에 따라 호스트 시스템을 세 가지 크기로 지정할 수 있습니다.
+
[cols="18,18,26,30"]
|===
| 시스템 크기 | CPU | RAM(스왑 메모리는 비활성화 상태여야 함) | 디스크 
 ...
 a| 
* 대형 *
| CPU 16개 | 64GB RAM | 500GiB SSD 켜짐/ 또는 * 100GiB를 /opt * 395GiB에서 사용 가능 /var * 5GiB에서 사용 가능 /tmp 
...
== 사내 Linux 호스트에 Data Sense를 설치합니다
...
.시작하기 전에
* Data Sense 소프트웨어는 한 번에 하나의 주요 버전으로 업그레이드할 수 있습니다. 예를 들어, 버전 1.20.x가 설치되어 있는 경우 1.21.x로 업그레이드할 수 있습니다 몇 가지 주요 버전이 뒤쳐지면 소프트웨어를 여러 번 업그레이드해야 합니다.
