...
.단계
이(가) 있어야 합니다 link:task-scanning-databases.html#add-the-database-server["하나 이상의 데이터베이스 서버가 추가되었습니다"^] 데이터 Fusion 소스를 추가하기 전에 BlueXP 분류를 수행합니다.
...
정규식을 작성하는 데 도움이 필요한 경우 을 참조하십시오 https://regex101.com/["정규식 101"^]. 원하는 맛을 내려면 * Python * 을 선택하여 BlueXP 분류가 정규식에서 일치하는 결과의 유형을 확인하십시오. 를 클릭합니다 https://pythonium.net/regex["Python Regex 테스터 페이지입니다"^] 또한 패턴의 그래픽 표현을 표시할 때 유용합니다.
