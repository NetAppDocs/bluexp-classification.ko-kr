...
summary: 몇 가지 단계를 완료하여 인터넷 액세스가 가능한 네트워크의 Linux 호스트 또는 클라우드의 Linux 호스트에 데이터 센스를 배포합니다. 
---
...
[role="lead"]
몇 가지 단계를 완료하여 인터넷 액세스가 가능한 네트워크의 Linux 호스트 또는 클라우드의 Linux 호스트에 데이터 센스를 배포합니다.
...
구내의 Linux 호스트에 설치하는 일반적인 구성 요소 및 연결은 다음과 같습니다.

image:diagram_deploy_onprem_overview.png["필요한 BlueXP 및 네트워크 구성 요소 및 관련 연결의 다이어그램"]

클라우드의 Linux 호스트에 설치하는 일반적인 구성 요소 및 연결은 다음과 같습니다.
...
커넥터가 없는 경우 https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-installing-linux.html["Connector를 온-프레미스에 배포합니다"^] 네트워크의 Linux 호스트 또는 클라우드의 Linux 호스트
...
| 대형 | CPU 16개 | 64GB RAM | /opt * 에서 500GB SSD 켜기/또는 * 100GB 사용 가능 * /var * 5GB에서 사용 가능 /tmp 


| 중간 | CPU 8개 | 32GB RAM | /opt * 에서 200GB SSD 켜기/또는 * 50GB 사용 가능 * 145GB는 /var * 5GB에서 /tmp에 사용 가능 


| 작은 크기 | CPU 8개 | 16GB RAM | /opt * 에서 100GB SSD 켜짐/ 또는 * 50GB 사용 가능 * /var * 5GB에서 사용 가능 * /tmp에서 사용 가능 
|===
+
...
* * AWS EC2 인스턴스 유형 *: 위의 CPU 및 RAM 요구 사항을 충족하는 인스턴스 유형입니다. 추천합니다 https://aws.amazon.com/ec2/instance-types/m5/["m5.4xLarge"^].
* * Azure VM size *: 위의 CPU 및 RAM 요구 사항을 충족하는 인스턴스 유형입니다. 추천합니다 https://docs.microsoft.com/en-us/azure/virtual-machines/dv3-dsv3-series#dsv3-series["standard_d16s_v3"^].
* * GCP 시스템 유형 *: 위의 CPU 및 RAM 요구 사항을 충족하는 인스턴스 유형입니다. 추천합니다 https://cloud.google.com/compute/docs/general-purpose-machines#n2_machines["N2-표준-16"^].
