...
이제 처음에 해당 권한으로 설정되지 않은 경우 NFS의 그룹 ID가 BlueXP 분류에서 직접 "Open to Organization"으로 간주되도록 구성할 수 있습니다. 자세한 내용은 를 참조하십시오 https://docs.netapp.com/us-en/bluexp-classification/task-add-group-id-as-open["추가 그룹 ID를 "조직에 열려 있음"으로 추가"].
