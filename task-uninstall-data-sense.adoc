...
. BlueXP  분류 페이지 맨 위에서 * 데이터 감지 제거 * 를 선택합니다image:button-gallery-options.gif["자세히 단추"].
+
image:screenshot_compliance_uninstall.png["BlueXP 분류 소프트웨어를 제거하는 단추 스크린샷."]

. 데이터 감지 제거 _ 대화 상자에서 * uninstall * 을 입력하여 BlueXP  커넥터에서 BlueXP  분류 인스턴스의 연결을 끊을 것임을 확인한 다음 * Uninstall * 을 선택합니다.
