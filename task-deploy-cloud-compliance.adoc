...
[role="quick-margin-para"]
BlueXP에서 BlueXP 분류 검사를 수행하는 첫 번째 500TB의 데이터는 30일간 무료로 제공됩니다. 해당 시점 이후에도 계속해서 데이터를 스캔하려면 클라우드 공급자 마켓플레이스 또는 NetApp의 BYOL 라이센스를 통한 BlueXP 구독이 필요합니다.
...
온프레미스 ONTAP 시스템, 비 NetApp 파일 공유, 일반 S3 오브젝트 스토리지 및 데이터베이스를 검사할 수 있습니다. 이러한 클라우드 커넥터를 사용할 경우
