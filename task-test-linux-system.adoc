...
가능합니다 https://["Connector On-Premises를 설치합니다"^] 네트워크의 Linux 호스트 또는 클라우드의 Linux 호스트 BlueXP 분류를 사내에서 설치하려는 일부 사용자는 Connector를 내부에 설치할 수도 있습니다.

클라우드 공급자 환경에 Connector를 만들려면 를 참조하십시오 https://["AWS에서 커넥터 생성"^], https://["Azure에서 커넥터 만들기"^], 또는 https://["GCP에서 커넥터를 생성하는 중입니다"^].
...
.단계
. 에서 BlueXP 분류 필수 구성 요소 스크립트를 다운로드합니다 https://["NetApp Support 사이트"^]. 선택해야 하는 파일의 이름은 * standalone-pre-requised-tester-<version> * 입니다.
